...
from WhyzuRobot.events import register
from WhyzuRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@WhyzuLogo", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
        fpath_ = glob.glob("./WhyzuRobot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [ᴡʜʏᴢᴜ ʀᴏʙᴏᴛ](https://t.me/WhyzuRobot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./WhyzuLogo/resources/fonts"):
            os.remove(font_)
...
async def logo_(event):
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [ᴡʜʏᴢᴜ ʀᴏʙᴏᴛᴀ](https://t.me/WhyzuRobot)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
    if os.path.exists(bg_):
        os.remove(bg_) 
    if os.path.exists(font_):
        if not font_.startswith("./WhyzuRobot/resources/fonts"):
            os.remove(font_)
